...
new=r'''function hjDualData(r){return r?.source_status?.selection_dual_light_v01||{}}
function hjDualDirectionBadge(r){
 const x=String(hjDualData(r).direction_light||r?.source_status?.final_selection_light||'').toUpperCase();
...
new_metric="metric('🎯 方向灯',hjDualDirectionBadge(r),'只回答Top1方向强弱；V/DTR/DLR/二审不得否决')+metric('💰 V灯',hjDualValueBadge(r),'只回答当前赔率价值，不否决方向')+metric('🎫 出手档位',hjDualActionBadge(r),'方向/价值/市场可执行性分开')"
...
new_detail="<div>🎯方向灯</div><div>'+esc(hjDualDirectionBadge(r))+'</div><div>💰V灯</div><div>'+esc(hjDualValueBadge(r))+'</div><div>🎫出手档位</div><div>'+esc(hjDualActionBadge(r))+'</div><div>普通HAD可执行</div><div>'+esc((hjDualData(r).had_execution_available===true||String(hjDualData(r).had_execution_available).toLowerCase()==='true')?'可执行':'暂不可执行/未确认')+'</div><div>风险层权限</div><div>DTR / DLR / GPT / 情报 / 让球 = 提示层，不一票否决方向</div>"
